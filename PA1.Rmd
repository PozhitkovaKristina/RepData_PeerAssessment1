...
```{r totalstep}
steps_day <- df%>%group_by(date)%>%summarise(total_steps_by_day=sum(steps, na.rm=TRUE))
```

2. Make a histogram of the total number of steps taken each day

```{r hist}
ggplot(steps_day,  aes(total_steps_by_day)) +   geom_histogram()
```
...
```{r stepsinterval}
steps_interval <- df%>%group_by(interval)%>%summarise(mean_steps_interval=mean(steps, na.rm=TRUE)) 
...
```{r totelstepsimp}
steps_day_imp <- df_imp%>%group_by(date)%>%summarise(total_steps_by_day=sum(steps, na.rm=TRUE))
...
After missing imputation (NA=mean by date) The mean = `r mean_steps_imp` and median = `r median_steps_imp` of the total number of steps taken per day.
